...
class OllamaService:
	...
	async def generate(self, prompt: str, model: Optional[str] = None, system: Optional[str] = None, max_tokens: Optional[int] = None) -> str:
		...
		logger.info(f"   📡 OLLAMA: Calling {model} API...")
		logger.info(f"      Prompt length: {len(prompt)} chars")
		if system:
			logger.info(f"      System prompt: {len(system)} chars")
		print(f"   📡 OLLAMA: Calling {model} API...")
		...
		try:
			async with httpx.AsyncClient(timeout=120.0) as client:
				...
				logger.info(f"   ✅ OLLAMA: Received response ({len(content)} chars)\n")
				print(f"   ✅ OLLAMA: Received response ({len(content)} chars)\n")
				return content
...
{{
  ...
  "reasoning": "brief one sentence explanation"
		...
		try:
			...
			logger.info("   ✅ OLLAMA: Successfully parsed JSON response")
			print(f"   ✅ OLLAMA: Successfully parsed JSON response")
			return parsed
		except json.JSONDecodeError as e:
			error_msg = f"   ❌ OLLAMA: Failed to parse JSON - {str(e)}\n      Response preview: {response[:300]}..."
			logger.error(error_msg)
