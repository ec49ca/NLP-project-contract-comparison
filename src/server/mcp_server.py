...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.error(f"Langfuse integration failed: {e}")
...
async def register_agent(agent_class_name: str, config: Dict[str, Any]):
    ...
    try:
        ...
        agent_module = __import__(
            f"src.agents.{agent_class_name}", fromlist=[agent_class_name]
        )
...
async def get_agent(agent_id: str):
    ...
    try:
        ...
        return {
            ...
            "status": (await registry.get_registry_state())["agents"][str(agent_uuid)][
                "status"
            ],
        }
    except ValueError:
...
async def execute_agent_capability(
    agent_name: str, capability: str, parameters: Dict[str, Any]
):
    """
