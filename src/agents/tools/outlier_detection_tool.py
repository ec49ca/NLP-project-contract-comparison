...
class OutlierDetectionTool:
    ...
    async def execute_tool(self, arguments: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            methods = arguments.get("methods", ["iqr", "zscore", "modified_zscore", "isolation_forest"])
            ...
            for field in target_fields:
                values_with_indices = self._extract_numeric_values_with_indices(data, field)
                
                if len(values_with_indices) < 10:
                    logger.warning(f"Insufficient data for field {field}: {len(values_with_indices)} values")
                    continue
                ...
                for method in methods:
                    if method == "iqr":
                        ...
                        field_analysis["methods"]["zscore"] = self._detect_outliers_zscore(
                            values_with_indices, thresholds.get("zscore_threshold", 3.0)
                        )
                    elif method == "modified_zscore":
                        field_analysis["methods"]["modified_zscore"] = self._detect_outliers_modified_zscore(
                            values_with_indices, thresholds.get("modified_zscore_threshold", 3.5)
                        )
                    elif method == "isolation_forest":
                        field_analysis["methods"]["isolation_forest"] = self._detect_outliers_isolation_forest(
                            values_with_indices, thresholds.get("isolation_forest_contamination", 0.1)
                        )
                
                # Calculate summary statistics for the field
                field_analysis["summary"] = self._calculate_outlier_summary(field_analysis["methods"])
            ...
            result["summary_stats"] = self._calculate_overall_summary(result["outliers"])
    ...
    def _extract_numeric_values_with_indices(self, data: List[Dict[str, Any]], field: str) -> List[Tuple[int, float]]:
    ...
    def _detect_outliers_iqr(self, values_with_indices: List[Tuple[int, float]], multiplier: float) -> Dict[str, Any]:
            ...
            for idx, value in values_with_indices:
                if value < lower_bound or value > upper_bound:
                    ...
                    outliers.append({
                        ...
                        "distance_from_bound": float(abs(value - (lower_bound if value < lower_bound else upper_bound)))
                    })
    ...
    def _detect_outliers_zscore(self, values_with_indices: List[Tuple[int, float]], threshold: float) -> Dict[str, Any]:
    ...
    def _detect_outliers_modified_zscore(self, values_with_indices: List[Tuple[int, float]], threshold: float) -> Dict[str, Any]:
            ...
            for idx, value in values_with_indices:
                ...
                if modified_z_score > threshold:
                    outliers.append({
                        ...
                        "modified_z_score": float(0.6745 * (value - median) / modified_mad),
    ...
    def _detect_outliers_isolation_forest(self, values_with_indices: List[Tuple[int, float]], contamination: float) -> Dict[str, Any]:
            ...
            for method_name, method_result in methods.items():
                if "error" not in method_result:
                    ...
                    summary["method_comparison"][method_name] = {
                        ...
                        "outlier_percentage": method_result.get("outlier_percentage", 0),
    ...
    def _find_global_outliers(self, outliers: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for field, field_analysis in outliers.items():
                for method_name, method_result in field_analysis.get("methods", {}).items():
                    if "error" not in method_result:
            ...
            multi_method_outliers = [
                outlier for outlier in global_outliers.values()
                if len(outlier["detected_by"]) > 1
            ]
            
            # Sort by number of methods that detected them
            multi_method_outliers.sort(key=lambda x: len(x["detected_by"]), reverse=True)
    ...
    def _calculate_overall_summary(self, outliers: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            summary = {
                ...
                "total_outliers": 0,
                "global_outliers": 0,
                "severity_distribution": {"extreme": 0, "high": 0, "medium": 0, "low": 0},
