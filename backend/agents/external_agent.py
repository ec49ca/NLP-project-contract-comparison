...
"""
    ...
    async def initialize(self, config: Dict[str, Any]) -> None:
        ...
        if not pinecone_api_key:
            logger.warning("⚠️  PINECONE_API_KEY not set - ExternalAgent will be disabled")
            logger.warning("    Set PINECONE_API_KEY and run 'python -m backend.data_extraction' to enable")
            return
        ...
        try:
            ...
            logger.warning("    ExternalAgent will be disabled. Run 'python -m backend.data_extraction' to create index.")
            return
        ...
        if not openai_api_key:
            logger.warning("⚠️  OPENAI_API_KEY not set - ExternalAgent will be disabled")
            return
    ...
    async def process_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not self._pinecone_index or not self._openai_client:
            return {
                "success": False,
                "error": "ExternalAgent not initialized. Please set PINECONE_API_KEY and OPENAI_API_KEY, then run 'python -m backend.data_extraction'"
            }
        ...
        print("      🔵 EXTERNAL AGENT: Embedding query...")
        query_embed = self._embed_query(query)

        logger.info("🔍 ExternalAgent: Running semantic search...")
        print("      🔍 EXTERNAL AGENT: Running semantic search...")
        matches = self._semantic_search(query_embed=query_embed, top_k=5)

        if not matches:
            logger.info("      ⚠️  EXTERNAL AGENT: No relevant WIPO documents found")
            print("      ⚠️  EXTERNAL AGENT: No relevant WIPO documents found")
            return {
                "success": True,
                "data": {
                    "query": query,
        ...
        print("      🧠 EXTERNAL AGENT: Calling LLM for summary...")
...
""",
        ...
        logger.info(f"      ✅ EXTERNAL AGENT: Got response ({len(llm_response)} chars)")
        print(f"      ✅ EXTERNAL AGENT: Got response ({len(llm_response)} chars)")
