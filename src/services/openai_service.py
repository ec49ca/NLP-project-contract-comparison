...
from .config import OPENAI_API_KEY, OPENAI_MODEL, OPENAI_MAX_TOKENS, OPENAI_TEMPERATURE, validate_openai_config
	...
	def _parse_entity_response(self, response: str) -> List[Dict[str, Any]]:
		...
		try:
			...
			for entity in entities:
				if all(key in entity for key in ["name", "type", "context", "confidence"]):
	...
	def _parse_relationship_response(self, response: str) -> List[Dict[str, Any]]:
		...
		try:
			...
			for rel in relationships:
				if all(key in rel for key in ["source", "source_type", "target", "target_type", "relationship", "confidence"]):
