...
class MultipleRegressionTool:
    ...
    async def execute_tool(self, arguments: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            try:
                result = self._multiple_regression(
                    X_train,
                    X_test,
                    y_train,
                    y_test,
            ...
            return {
                ...
                "matched_kwargs": {
                    ...
                    "includeInteractions": (
                        include_interactions
                        if "include_interactions" in locals()
                        else False
                    ),
    ...
    def _multiple_regression(
        ...
        try:
            ...
            if include_interactions and len(feature_names) > 1:
                ...
                X_train_with_interactions = np.column_stack(
                    [X_train] + interaction_features
                )
                X_test_with_interactions = np.column_stack(
                    [X_test]
                    + [
                        X_test[:, i] * X_test[:, j]
                        for i in range(len(feature_names))
            ...
            adjusted_r2_test = 1 - (1 - r2_test) * (len(y_test) - 1) / (
                len(y_test) - p - 1
            )
            ...
            diagnostics = {
                ...
                "significant_features": [
                    final_feature_names[i]
                    for i in range(len(final_feature_names))
                    if p_values[i] < 0.05
                ],
            ...
            if include_interactions:
                interaction_terms = [
                    name for name in final_feature_names if " * " in name
                ]
                interaction_analysis = {
                    "interaction_terms": interaction_terms,
                    "interaction_coefficients": {
                        name: model.coef_[final_feature_names.index(name)]
