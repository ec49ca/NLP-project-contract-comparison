...
class OpenAIService(LLMService):
	...
	async def generate_stream(self, prompt: str, model: Optional[str] = None, system: Optional[str] = None, max_tokens: Optional[int] = None) -> AsyncIterator[str]:
		...
		try:
			...
			error_msg = f"   ❌ LLM STREAM ERROR (OpenAI): {str(e)}\n"
			logger.error(error_msg)
			print(error_msg)
			raise Exception(f"Failed to stream response from OpenAI: {str(e)}")
		except httpx.HTTPStatusError as e:
			error_msg = f"   ❌ LLM STREAM HTTP ERROR (OpenAI): {e.response.status_code} - {str(e)}\n"
			logger.error(error_msg)
			print(error_msg)
			raise Exception(f"Failed to stream response from OpenAI: HTTP {e.response.status_code}")
	...
	async def generate_json(self, prompt: str, model: Optional[str] = None, system: Optional[str] = None) -> Dict[str, Any]:
		...
		"""
		# Check if system prompt already specifies a JSON format
		# If it contains format indicators, don't override it
		system_lower = (system or "").lower()
		has_custom_format = (
			"needs_internal_agent" in system_lower or 
			"needs_external_agent" in system_lower or
			'"needs_' in system_lower
		)
		
		if has_custom_format:
			# System prompt already specifies format - just ask for JSON, don't override format
			json_prompt = f"""{prompt}

IMPORTANT: Respond with ONLY valid JSON, no other text. Use the exact format specified in the system prompt above."""
		else:
			# Default format for legacy orchestrator compatibility
			json_prompt = f"""{prompt}
